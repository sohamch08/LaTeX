...
{\begin{tikzpicture}[remember picture,overlay]
	\node[anchor=north west,inner sep=0pt,opacity=0.3] at (0,0) {\includegraphics[width=\paperwidth]{euler (1).jpg}}; % Background image
	\draw[anchor=north] (midpoint) node [fill opacity=0,text=white,text opacity=1,inner sep=1cm]{\centering a}; % Author name
	\end{tikzpicture}};
\end{tikzpicture}
	\begin{center}
		\thispagestyle{empty}
