...
\end{align*}Hence $$	\int_{|z|=\rho}\frac{|dz|}{|z-a|^4}=-i\rho \int_{|z|=\rho}\frac{zdz}{(z-a)^2\lt(\rho^2-\ova z\rt)^2}=-i\rho \lt[ 2\pi i \, \frac{\rho^2+|a|^2}{(\rho^2-|a|^2)^3} \rt]=2\pi\rho\, \frac{\rho^2+|a|^2}{(\rho^2-|a|^2)^3}$$
	...
	\solve{
		%Solution
		Given $|f(z)|<|z|^n$. ITo show $f$ is a polynomial it is enough to show $\exs\ n\in\bbN$ such that $f^{(n)}(z)=0$. We have $$f^{(n)}(z)=\frac{n!}{2\pi i}\int_{|s|=\rho}\frac{f(s)ds}{(s-z)^{n+1}}$$Now if $|z|>\rho$ then
		\begin{align*}
