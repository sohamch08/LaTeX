...
Now exercise
\exs{}{\label{exs1}Prove that triangle inequality is true if $p\geq 1$ for $p-$norms. (What goes wrong for $p<1$ ?)}
...
\thm{}{\label{thm1}For a finite dimensional normed linear space, notion of convergence is independent of norm chosen}

\begin{myproof} \textbf{For Property \ref{n:3} for norm-2}	\subsubsection*{\textbf{When field is $\bbR:$}} We have to show\begin{align*}
...
Now there is no reason why we can not consider a norm on an infinite dimensional vector space. It will work. Perhaps i can define only for some sequences where the some converges. \ex{Example of Theorem 1.\ref{thm1}}{Suppose for set of all bounded infinite sequences a vector space because every number in a vector is less than some number so if you add two vectors then add then bound and if you scale then scale the bound. Now the $\infty$ norm works on that.
...
\exs{}{\label{exs2}Modify the above proof for field $\bbC$}
\exs{}{\label{exs3}Show that the following are normed linear spaces.\begin{enumerate}[label=(\alph*)]
